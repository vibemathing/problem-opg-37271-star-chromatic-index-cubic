...
The self-contained companion uses standard-library CPython 3.13.5, one thread, a 15-second wall alarm, CPU limits 15/16 seconds and a 512 MiB address-space cap. The checker tests 96 concrete end colorings for t=1,...,24, all 1536 ordered outside palette cases for one four-rung fixture, 54 further periodic boundary smoke cases, and four rejected mutations. It freezes 256 normalized positive palette recipes and the complete head-component table. These are generator-side checks, not a trusted receipt, repository-script execution, or an exhaustive search over all graphs. Finite prefix success alone would not imply the all-length claim; Section 2's rung classification supplies that missing argument.
