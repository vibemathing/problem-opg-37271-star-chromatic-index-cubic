...
The certificate encodes 200 normalized fifteen-edge colorings using 120 five-bit eligibility masks, five normalized matching words and five explicit cycle-color templates. Applying all six permutations of matching colors 1,2,3, while fixing 4,5,6, recovers all 1,200 rows. Every eligible matching word uses all three colors, so every orbit has size six. The constructor checks equality of the expanded and uncompressed tables and fidelity of the template encoding; the replay checks complete expanded coverage directly. No opaque compressed payload or coloring search is needed to decode a witness. For each row the first five edges form the specified matching and retain its colors. The replay does not import the construction or search for colors: it enumerates all 120*243 prospective inputs to determine eligibility, checks exact table coverage, reconstructs the actual graph, and tests all simple four-edge paths and four-cycles from adjacency lists.
...
Observed local runtime: CPython 3.13.5; standard library; one thread. Each script enforces a 35-second wall alarm, 35/36-second CPU limits, and 512 MiB address-space limit. Certificate input and generated certificate are bounded by 262,144 bytes; the table has 1,200 expanded rows. These are generator-domain diagnostics, not an admitted adapter, repository command, kernel run, or trusted receipt. The profile's command_execution declaration has not been changed.
